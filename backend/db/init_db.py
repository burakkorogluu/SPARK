...
def seed_transformers():
    ...
    if not db.query(models.Reactor).first():
        reactors = [
            models.Reactor(id="RCT-UMR-1", name="TM-A Reaktör R1", current_transformer_id="UMR-TRA", alternative_transformer_id="UMR-TRB", capacity_kvar=500.0, status="active"),
            models.Reactor(id="RCT-KRT-1", name="TM-B Reaktör R1", current_transformer_id="KRT-TRB", alternative_transformer_id="KRT-TRA", capacity_kvar=350.0, status="active"),
        ]
        db.add_all(reactors)
